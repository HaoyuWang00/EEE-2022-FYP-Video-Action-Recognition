...
class wget_downloader:
    ...
    def downloadFile(url, location=""):
        ...
        # wget.download(url, out = location, bar = self.progressBar)
...
class EpicDownloader:
    ...
    def download_file(url, output_path):
        ...
        try:
            ...
            # except Exception as e:
            #     print('Retrying failed, from {}\nError: {}'.format(url, str(e)))
            
            # wget.download(url, output_path)
            arg_list = ["wget", "-O", str(output_path), str(url)]
            subprocess.run(arg_list, shell=True, check=True)
