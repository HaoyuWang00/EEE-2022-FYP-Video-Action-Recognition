...
dataset_replacement_path = '/home/students/s121md105_01/kinetics-dataset/k400/replacement/replacement_for_corrupted_k400'
...
if __name__ == '__main__':
    ...
    for (direcotry_input, filename_input) in zip(directories_input, files_input):
        ...
        video_list = os.listdir(dataset_replacement_path)
        for vid_file_path in video_list:
            # trunk the name
            vid_code = vid_file_path[:11]
            # check if the video belongs to this split
            if vid_code in folders:
                idx = folders.index(vid_code)
                cat_idx = idx_categories[idx]
                cat_name = list(dict_categories.keys())[list(dict_categories.values()).index(cat_idx)]
                cat_folder_path = os.path.join(output_frame_path_split, cat_name)
                ...
                original_path = os.path.join(dataset_replacement_path, vid_file_path)
                destination_path = cat_folder_path
                # check if the corrupted file is already in the desitation
                if vid_file_path in os.listdir(destination_path):
                    print("Corrupted file " + vid_file_path + " already exists in " + destination_path)
                    print("REPLACING... ...")
                    dest_file_name = os.path.join(destination_path, vid_file_path)
                    shutil.move(original_path, dest_file_name)
                else:
                    print('MOVING ' + vid_file_path + ' to ' + cat_folder_path)
                    shutil.move(original_path, destination_path)
            else:
                print(vid_code + " does not belong to " + direcotry_input)
