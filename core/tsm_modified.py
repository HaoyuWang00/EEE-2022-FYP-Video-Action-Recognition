...
from ops.temporal_shift_modified import make_temporal_interlace
...
class TSM_MODIFIED(nn.Module):
    def __init__(
        ...
        super(TSM_MODIFIED, self).__init__()
    ...
    def _prepare_base_model(self, base_model, config={}):
        ...
        if self.pretrained and backbone_pretrained is not None:
            logger.info(f"Loading backbone model with {backbone_pretrained} weights")
        elif self.pretrained is None and backbone_pretrained is None:
            logger.info("Randomly initialising backbone")
    ...
    def train(self, mode=True):
        ...
        :return:
        """
        super(TSM_MODIFIED, self).train(mode)
        count = 0
